...
export default function Quote() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const { error } = await supabase.from("Quotes").insert([
      { firstName: firstName, lastName: lastName, phone, email, service, message },
    ]);

    if (error) {
      alert("Error submitting quote: " + error.message);
    } else {
      // TODO: Send auto-reply email & company email (using Supabase functions or serverless)
  ...
  return (
    <div className="min-h-screen bg-zinc-50 px-6 py-10 text-zinc-900 dark:bg-black dark:text-zinc-100">
      <div className="mx-auto max-w-xl">
        <Link href="/" className="mb-6 inline-flex items-center text-sm text-zinc-500 hover:text-zinc-700 dark:text-zinc-400">
          ← Back
        </Link>
        ...
        <form onSubmit={handleSubmit} className="space-y-6 rounded-2xl border border-zinc-200 bg-white p-6 shadow-sm dark:border-zinc-800 dark:bg-zinc-950">
